...
from smoothing.constants import MIN_STARTS_THRESHOLD, INITIAL_PARAMS, BOUNDS, N_SAMPLES, N_TRIALS


def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    df = df.groupby('trainer_name').agg({
        'dnf': 'sum',
...
def generate_beta_binomial_samples(n: int, alpha: float, beta: float, size: int) -> np.ndarray:
...
def create_sample_plot(alpha0: float, beta0: float) -> None:
    ...
    plt.figure()
    plt.hist(samples, bins=np.arange(0, N_TRIALS/10) - 0.5, density=False, alpha=0.75, color='blue', edgecolor='black')
...
def create_smoothed_scatter_plot(df: pd.DataFrame, alpha0: float, beta0: float) -> None:
    ...
    plt.figure()
    plt.scatter(df['dnf_pct'], df['smoothed_dnf_pct'], c=np.log(df['n_starts']), cmap = 'bwr', alpha=0.5)
    plt.plot([0, .15], [0, .15], color='gray', linestyle='--', alpha=0.8)
...
def create_ranking_plot(df: pd.DataFrame, alpha0: float, beta0: float) -> None:
    ...
    plt.figure()
    plt.errorbar(df['smoothed_dnf_pct'], df['trainer_name'], 
                xerr=[df['smoothed_dnf_pct'] - df['low'], 
                    df['high'] - df['smoothed_dnf_pct']], 
